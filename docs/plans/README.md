...
- `plan-v1.md`는 최초 승인된 기획 문서다. 계획 변경은 기본적으로 새 델타 파일(`plan-v2.md`,
  `plan-v3.md`, ...)을 추가하는 방식을 따르되, 필요하면 `plan-v1.md` 자체도 직접 수정할 수
  있다(2026-09-18, plan-v16 참고 — 과거 "절대 직접 수정 금지" 정책은 폐지됨).
- 델타 파일을 추가하는 경우, 각 파일 맨 위에 이전 버전 대비 무엇이 왜 바뀌었는지 요약을 남긴다.
- 어떤 버전이 "현재 유효한 계획"인지는 이 README에서 아래 표로 추적한다.

| 버전 | 파일        | 상태                               | 비고                                                                                                                                                                   |
| ---- | ----------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1   | plan-v1.md  | 유효(기반, 2026-09-18 직접 수정됨) | 최초 승인본 — 실행 불변식 섹션 삭제, "취업 포트폴리오"→"실무 적용 수준 포트폴리오" 문맥 수정(v16 참고)                                                                 |
...
| v16  | plan-v16.md | 유효(델타)                         | plan-v1 "절대 직접 수정 금지" 정책 폐지 + "취업 포트폴리오"→"실무 적용 수준 포트폴리오" 문맥 전환                                                                      |
| v17  | plan-v17.md | 유효(델타)                         | `/clear` 타이밍 가이드라인. task/TASK-56-clear-timing 브랜치에 2026-09-12부터 미병합 방치되어 있던 것을 TASK-67로 복구(원래 plan-v16이었으나 번호 충돌로 v17로 재배정) |
