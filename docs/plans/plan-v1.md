...
**최종 목적은 개발자 취업을 위한 포트폴리오**다. 다만 흔한 CRUD 토이 프로젝트가 아니라, 실제 스테이 랜딩 페이지처럼 느껴지는 제품(포트폴리오 겸 숙소 마케팅 페이지)을 만들어 스크롤을 따라가면 배경 경험→운영 철학까지의 서사가 드러나는 구조로 차별화한다. "현장 경험을 소프트웨어 요구사항으로 번역했다"는 스토리 자체가 채용 담당자에게 어필하는 지점이다. 현재 실제 사진·시공 과정·운영 데이터(점유율, 리뷰 등)는 준비되어 있지 않으므로, 이번 구현은 **현실적인 더미(가짜) 콘텐츠**로 전체 구조를 채우고, 추후 실제 자료로 교체 가능하게 데이터 구조를 분리해서 설계한다.
...
## README / 기술 문서 (채용 담당자용)

개발자 포트폴리오 목적을 위해 저장소 루트에 `README.md`를 작성한다: 프로젝트 한 줄 소개, 라이브 데모 링크, 아키텍처 다이어그램(프런트/백엔드/DB/AWS 구성), 주요 기술 선택 이유(왜 PostgreSQL·JWT·EC2·ArchUnit인지), 테스트 전략, 스크린샷(공개 페이지 + 대시보드). 코드만으로 드러나지 않는 "판단 과정"을 문서화해 AI가 코드를 생성했더라도 본인이 의사결정하고 검증했다는 근거를 남긴다.
...
사용자가 명시적으로 "절대 바꾸지 말라"고 지정한 실행 규칙:

1. **백로그 leaf task = 파일 1개**: backlog의 최종 말단(leaf) task는 프런트엔드/백엔드 구분 없이 항상 정확히 파일 1개만 수정한다. 이 문서의 52개 상위 draft(예: "Room 엔티티+API+관리화면")는 여러 파일에 걸치므로 그 자체로는 leaf가 아니다 — promote 후 실행 시점에 `backlog task create -p <부모ID> --modified-file <파일경로>`로 파일 1개짜리 leaf subtask들로 반드시 다시 쪼갠다.
2. **leaf task 실행 = 서브에이전트(필수)**: 토큰 소모를 감안해, 각 leaf task의 실제 구현은 오케스트레이팅 세션이 직접 하지 않고 항상 서브에이전트에 위임한다.
3. **Backlog.md CLI를 적극 활용**: 위 계층(상위 draft/task → 파일 1개 leaf subtask)을 관리할 때 `-p`(parent), `--modified-file`, `--depends-on` 등 CLI 기능을 실제로 사용한다.

이 세 가지는 도구 사용 방식과 동급의 불변식으로 취급하며, 이후 어떤 이유로도 변경하지 않는다.
